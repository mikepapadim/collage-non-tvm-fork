...
def resnet_block(input, strides, out_channels, name):
    ...
    if (strides[2]>1) or (input.shape[1] != out_channels * 4):
        input = make_conv2d(input_tensor=input, filter_shape=(1,1,input.shape[1],out_channels*4), strides=strides, padding="SAME", actimode="RELU", name=name+"_conv4")
...
if __name__ == '__main__':
    hw, network = 'rtx2070', 'resnet50'
    input_shape = (1, 64, 56, 56)
    inputs = np.random.uniform(-1, 1, size=input_shape).astype("float32")

    method_name = 'TF'
    measure_tf2_gpu(resnet50_tf2, inputs, method_name, hw, network)

    # This errors out; resize kernel is not supported even by the most recent XLA
    method_name = 'TF-XLA'
    measure_tf2_gpu(resnet50_tf2_xla, inputs, method_name, hw, network)
